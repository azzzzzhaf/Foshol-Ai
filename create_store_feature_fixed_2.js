...
// Safe injection
const storeBtnSafe = '<button onclick="switchTab(\\'store\\')" class="nav-btn w-full text-left px-4 py-3 rounded-xl font-bold transition text-green-100 hover:bg-green-800" data-tab="store"><i class="fa-solid fa-store w-6 text-blue-400"></i> Manage Store</button>\n            ';

if(!adminHtml.includes('data-tab="store"')) {
    // wait I used backslash quote here again. Let's fix this in JS string literal.
    // In node, a backslash inside a string literal escapes the next char. I want it to literally be 'store' in the output.
    // Let me just avoid quotes entirely by using backticks.
    let buttonHtml = \`<button onclick="switchTab('store')" class="nav-btn w-full text-left px-4 py-3 rounded-xl font-bold transition text-green-100 hover:bg-green-800" data-tab="store"><i class="fa-solid fa-store w-6 text-blue-400"></i> Manage Store</button>\n            \`;
    adminHtml = adminHtml.substring(0, sidebarNavEnd) + buttonHtml + adminHtml.substring(sidebarNavEnd);
}
...
files.forEach(file => {
    ...
    if (file === 'index.html') {
        const oldStoreBtnEn = 'onclick="openServiceChat(\\'I want to join the waitlist for the E-Commerce store.\\', \\'আমি আসল সার-বীজ কেনার ই-কমার্স স্টোরের ওয়েটলিস্টে যুক্ত হতে চাই।\\')"';
        if(html.includes(oldStoreBtnEn)) {
            html = html.replace(oldStoreBtnEn, 'onclick="window.location.href=\\'/store\\'"');
