...
// 1. CREATE store.html
let storeHtml = readFile('about.html');
// We want to replace everything between <!-- Page Header --> and <!-- Footer -->
const headerStart = storeHtml.indexOf('<!-- Page Header -->');
const footerStart = storeHtml.indexOf('<!-- Footer -->');

const storeContent = `<!-- Page Header -->
...
storeHtml = storeHtml.substring(0, headerStart) + storeContent + storeHtml.substring(footerStart);
// Add Store Logic to store.html before closing body
const storeScript = `
    <script>
...
storeHtml = storeHtml.replace('</body>', storeScript);
...
const storeBtnStr = '<button onclick="switchTab(' + "'store'" + ')" class="nav-btn w-full text-left px-4 py-3 rounded-xl font-bold transition text-green-100 hover:bg-green-800" data-tab="store"><i class="fa-solid fa-store w-6 text-blue-400"></i> Manage Store</button>\n            ';

if(!adminHtml.includes('data-tab="store"')) {
    adminHtml = adminHtml.substring(0, sidebarNavEnd) + storeBtnStr + adminHtml.substring(sidebarNavEnd);
}
...
files.forEach(file => {
    ...
    const waitlistBtnTextEn = 'onclick="openServiceChat(' + "'I want to join the waitlist for the E-Commerce store.', 'আমি আসল সার-বীজ কেনার ই-কমার্স স্টোরের ওয়েটলিস্টে যুক্ত হতে চাই।'" + ')"';
    const linkStr = 'onclick="window.location.href=' + "'/store'" + '"';

    if(html.includes(waitlistBtnTextEn)) {
        html = html.replace(waitlistBtnTextEn, linkStr);
