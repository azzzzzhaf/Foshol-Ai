...
files.forEach(file => {
    ...
    // We only modify files that have the navbar
    ...
    if (mobileStartIdx !== -1) {
        // The mobile menu ends before the first <section> or <!-- Page Header --> or similar.
        // It's basically the </div> that closes the #mobile-menu block.
        // A safe way is to find the next section tag or comment and cut there, or just regex it.
        const idMobileStart = html.indexOf('<div id="mobile-menu"');
        if (idMobileStart !== -1) {
            let nextPart = html.substring(idMobileStart);
            let endingDivIdx = nextPart.indexOf('</div>\n    </div>') !== -1 ? nextPart.indexOf('</div>\n    </div>') + 13 : nextPart.indexOf('</div>\n') + 7; 
            // Better: find the end of the mobile block by looking for the next major block
            let endOfMobile = html.indexOf('<!--', idMobileStart + 10);
            if (endOfMobile === -1) endOfMobile = html.indexOf('<div', idMobileStart + 100);
            
            // Actually, I can just find the end of mobile menu manually.
            // Let's replace from mobileStartMarker to the end of its div.
            let mobileEndStr = '    </div>\n';
    ...
    // 3. Statically apply active classes based on file name
    const currentPath = file === 'index.html' ? '/' : '/' + file.replace('.html', '');
    
    // Make sure we have the exact targets to replace for active states
